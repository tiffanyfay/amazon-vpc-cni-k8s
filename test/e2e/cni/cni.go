...
var (
	...
	ns            *corev1.Namespace
	promResources *shared.Resources
	prom          *shared.Prom
	promAPI       promv1.API
)
...
var _ = Describe("cni-tester", func() {
	...
	BeforeEach(func() {
		...
		// promResources = shared.NewPromResources(promReplicas)
	...
	Context("prom should pass", func() {
		It("should get no ipamd errors", func() {
			log.Info("hi")
			received, err := promAPI.Query(ctx, "awscni_ip_max", prom.TestTime)
			Expect(err).NotTo(HaveOccurred())
			...
			dnsRequestFailurePercent, err := prom.Query("cni_test_external_http_request_total", "cni_test_dns_request_failure")
			Expect(err).NotTo(HaveOccurred())
